...
// Ensure public/api folder exists
...
// High-fidelity fallback mock data representing 60+ major Taiwan Stocks
const fallbackMockStocks = [
  ...
  { Code: "3711", Name: "日月光投控", ClosingPrice: "154.50", Change: "2.50", TradeVolume: "8124000", TradeValue: "1255158000", PEratio: "15.6", DividendYield: "3.37", PBratio: "2.1", Category: "半導體", OpeningPrice: "152.00", HighestPrice: "155.00", LowestPrice: "155.00", Transaction: "4820" },
...
async function fetchTaiwanStockData() {
  ...
  // Force reject unauthorized to bypass Node.js TLS warning for TWSE servers
  process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
  
  try {
    // 1. 獲取當日無延遲的收盤大盤行情 JSON (更新時間為每日 13:50 盤後)
    ...
    // 2. 獲取本益比、殖利率及淨值比 (OpenAPI) 作為基本面補強 (容許一日延遲)
    ...
    console.error('Error fetching live TWSE MI_INDEX, falling back to high-fidelity mock data:', error.message);
    return {
      success: true,
      source: 'fallback_mock',
      timestamp: new Date().toISOString(),
      count: fallbackMockStocks.length,
      data: fallbackMockStocks
    };
  }
}
